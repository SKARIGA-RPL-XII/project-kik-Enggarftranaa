...
<head>
    ...
    <style>
        ...
        .text-denda { color: #dc3545; font-weight: 800; font-size: 0.9rem; }
    </style>
</head>
<body>
...
<nav class="navbar bg-white border-bottom py-3 mb-5">
    <div class="container">
        <a class="navbar-brand" href="{{ url('user/dashboard') }}">Treasure <span class="text-primary">Library</span></a>
        <a href="{{ url('user/dashboard') }}" class="btn btn-outline-dark rounded-pill px-4 fw-bold small">
...
<tbody>
    ...
    @php
        ...
        // Cek kondisi terlambat: Status masih 'dipinjam' DAN hari ini sudah lewat tgl kembali
        $is_terlambat = ($p->status == 'dipinjam' && $hari_ini->gt($tgl_kembali));
        
        $selisih_hari = $is_terlambat ? $hari_ini->diffInDays($tgl_kembali) : 0;
    ...
    <tr>
        ...
        <td><span class="fw-600 {{ $is_terlambat ? 'text-danger fw-bold' : '' }}">{{ $tgl_kembali->format('d M Y') }}</span></td>
        <td>
            @if($p->status == 'dipinjam')
                @if($is_terlambat)
                    <span class="badge bg-danger text-white badge-status text-uppercase">TERLAMBAT</span>
                @else
                    <span class="badge bg-primary-subtle text-primary badge-status text-uppercase">SEDANG DIPINJAM</span>
                @endif
            @else
                <span class="badge bg-success-subtle text-success badge-status text-uppercase">DIKEMBALIKAN</span>
            @endif
        </td>
        <td>
            @if($is_terlambat)
                <span class="fw-bold text-danger">Rp {{ number_format($denda, 0, ',', '.') }}</span>
                <div class="small text-danger">{{ $selisih_hari }} hari</div>
            @else
